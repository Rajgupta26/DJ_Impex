...
153. **There is now an admin panel at `/admin`, backed by JSON files rather than a database.** The agency asked for it: image gallery, blog manager and enquiries, reading and writing `data/images.json`, `data/blogs.json` and `data/enquiries.json` through `fs/promises`, with uploaded files landing in `public/uploads`. No SQL, no NoSQL, no external service, by instruction.
     ...
     **The JSON files do not survive on Vercel.** The site deploys there, and Vercel's filesystem is read-only outside `/tmp` and rebuilt on every deploy. Uploads, edits and the enquiry copies will either fail outright or vanish at the next push. The panel is built for a host with a real disk. Either the panel moves to such a host, or the storage constraint has to give -- the agency chose the constraint, so it is theirs to revisit.
