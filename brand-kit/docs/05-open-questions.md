...
154. **The blog manager and the public journal are two separate stores.** `/journal` is built from the MDX in `brand-kit/content/journal`, with front matter, SEO fields and reading time; the panel writes plain Markdown rows to `data/blogs.json` and nothing reads them. A post created in the panel will not appear on the website. Connecting them means choosing which one is the source of truth -- and if it is the panel, the MDX pipeline's SEO fields, categories and cover-image handling need somewhere to live. Asked, not assumed.
