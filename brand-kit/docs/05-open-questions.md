...
144. **The preloader is drawn in CSS: a glass disc with a silver arc travelling round it, over a dark blurred page.** Third version in two days -- a fabric swatch that unfolded, then a compact disc of film, then the full-bleed Silk Reveal -- and this one, to the agency's brief of 2026-09-24, uses no video at all. `preloader.mp4` is deleted; it is in git at e8f811b.

     The ring is a conic gradient masked to a 2px annulus, which is how it fades from nothing to bright along its own length; a border can only be one flat colour per side. Monochrome by instruction. The spin sits inside a `prefers-reduced-motion: no-preference` query, so a viewer who asked for less motion gets the same ring standing still rather than an empty disc.

     Verified against the brief rather than by eye: z-index 9999, `rgba(15, 15, 15, 0.85)`, `blur(12px)`, a 220px disc at `border-radius: 50%` with `rgba(255,255,255,0.05)` on a `1px solid rgba(255,255,255,0.15)` border, ring animation `3s linear infinite`. Exit measured at scale 1.001 -> 1.030 as opacity runs 0.97 -> 0. Present at 289ms after navigation, unmounted at 1691ms, nothing left at z-9999, body `overflow` restored, and `elementFromPoint` at the centre of the screen returns the page rather than the overlay, so nothing is swallowing clicks.

     Two departures from the brief, both small. The tagline sits below the disc, not inside it: within a 220px circle, under a mark that is itself two lines, it would have had to set at about 10px, smaller than anything else on the site. And a 900ms floor is kept, because Next usually fires `load` before React hydrates and without it the whole thing is over inside about 200ms.

     The tagline is still Title Case as supplied while the home page's H1 is the same phrase in sentence case, which is the house rule. The site carries it both ways.
