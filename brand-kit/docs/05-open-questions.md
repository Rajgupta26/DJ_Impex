...
## /about rebuilt (premium pass)

78. **The opening statement is proposed copy.** "Sourcing, manufacturing, supplying and trading fine cloth from Mumbai since 1995", with "D J Impex & Co. trades from Mangaldas Market in Mumbai and exports under the brand Nabeen® to Africa and the Middle East" beneath it. Every element is the client's own: 1995 and the markets from `site.json`, Mangaldas Market from the address block (both candidate addresses agree on it), and the four activities from the client's sentence "from sourcing and manufacturing to supplying and trading". It is in `about.md` under "The house" and can be changed on its own.
79. **The promise band is set light, against the pattern.** Every other dark band on the site is white type over a navy wash. This one is navy type over a pale scrim, because the photograph is a white jacquard and a wash heavy enough to carry white type turned the frame into flat navy with the weave invisible. Its eyebrow is slate, not gold: gold on a light ground measures 2.88:1 and fails AA (question 31).
